...
def calc_volley(shield,armor,mod,weaponlist,custom_weapons=None,compdmg=0,n=11):
    ...
    # Print Output
    #print("Shield")
    #print(final_shield.transpose())
    #print("Armor")
    #print(final_armor.transpose())
    #print("Comp Dmg")
    #print(final_comp.transpose())
    ...
    plt.axhline(y=armor0, color='red', linestyle='--', linewidth=1)
    ...
    ymax = (shield0+armor0)
    plt.ylim(ymin,ymax)
    plt.axhspan(0, armor0, facecolor='lightsalmon', alpha=0.3, label='Low range')
    plt.axhspan(armor0, shield0+armor0, facecolor='lightblue', alpha=0.3, label='Mid range')
...
#display_single_shot(shield=1443*adjust_dict['heavy'],armor=1827,mod='fighter_laser',wpn='W0')


#a = calc_single(shield=2443*adjust_dict['heavy'],armor=1827,mod='fighter_laser',wpn='W0')[0]
#b = calc_single(shield=2443*adjust_dict['heavy'],armor=1827,mod='fighter_laser',wpn='W0')[1]
#calc_single_test(shield=a,armor=b,mod='fighter_laser',wpn='W8')


#expand_test(np.array([[1],[4]]),3)


#display_single_shot(shield=2200*adjust_dict['heavy'],armor=1827,mod='fighter_laser',wpn='W0')

#display_single_shot_v1(shield=2200*adjust_dict['heavy'],armor=1827,mod='fighter_laser',wpn='W0')

#calc_single_test(shield=np.array([2443*adjust_dict['heavy']]),armor=np.array([1827]),mod='fighter_laser',wpn='W0')



#a=calc_single_v1(shield=np.array([200*adjust_dict['heavy']]),armor=np.array([100]),mod='fighter_laser',wpn='W0')


#calc_volley(shield=2200*1.75,armor=4000,mod='fighter_laser',weaponlist=['W0','W0'],n=15)
#display_single_shot_v1(shield=2200*adjust_dict['heavy'],armor=1000,mod='fighter_laser',wpn='W0')

#display_single_shot_v1(shield=2200*adjust_dict['heavy'],armor=1000,mod='fighter_laser',wpn=custom_weapons[0])

#print(df.index.get_level_values('Tier').unique().tolist())

calc_volley(shield=2200*1.75,armor=4000,mod='fighter_laser',weaponlist=['W0'],n=15)


'''

Need a generic "display" function to display any output

What about some sort of IQR output


Maybe create a class which is defender (has components, adjust etc)
Then functions can be shots against it
'''